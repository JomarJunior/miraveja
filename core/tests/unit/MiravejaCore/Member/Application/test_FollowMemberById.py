...
from MiravejaCore.Member.Application.FollowMemberById import FollowMemberByIdCommand, FollowMemberByIdHandler
...
class TestFollowMemberByIdCommand:
    """Test cases for FollowMemberByIdCommand model."""

    def test_InitializeWithValidData_ShouldSetCorrectValues(self):
        """Test that FollowMemberByIdCommand initializes with valid data."""
        # Arrange
        agentId = MemberId.Generate()
        memberIdToFollow = MemberId.Generate()

        # Act
        command = FollowMemberByIdCommand(agentId=agentId, memberIdToFollow=memberIdToFollow)

        # Assert
        assert command.agentId == agentId
        assert command.memberIdToFollow == memberIdToFollow


class TestFollowMemberByIdHandler:
    """Test cases for FollowMemberByIdHandler application service."""

    def test_InitializeWithValidDependencies_ShouldSetCorrectProperties(self):
        """Test that FollowMemberByIdHandler initializes with valid dependencies."""
        # Arrange
        ...
        # Act
        handler = FollowMemberByIdHandler(
    ...
    @pytest.mark.asyncio
    async def test_HandleWithBothMembersExisting_ShouldFollowMember(self):
        """Test that Handle follows member when both members exist."""
        # Arrange
        agentId = MemberId.Generate()
        memberIdToFollow = MemberId.Generate()
        ...
        memberToFollow = Member.Register(
            id=memberIdToFollow,
        ...
        mockRepository.FindById.side_effect = [agent, memberToFollow]
    ...
    @pytest.mark.asyncio
    async def test_HandleWithMemberToFollowNotFound_ShouldNotRaiseException(self):
        """Test that Handle returns gracefully when member to follow does not exist."""
        # Arrange
        agentId = MemberId.Generate()
        memberIdToFollow = MemberId.Generate()
        ...
        mockDatabaseManager = Mock(spec=IDatabaseManager)
        mockRepository = Mock(spec=IMemberRepository)
        mockRepository.FindById.side_effect = [agent, None]  # Agent exists, member to follow does not
        mockDatabaseManager.GetRepository.return_value = mockRepository
        mockDatabaseManager.__enter__ = Mock(return_value=mockDatabaseManager)
        mockDatabaseManager.__exit__ = Mock(return_value=None)
    ...
    async def test_HandleWithValidMembers_ShouldLogCorrectMessages(self):
        ...
        assert "Following member by ID with command:" in firstInfoMessage
