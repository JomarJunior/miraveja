...
from MiravejaApi.Shared.DI.Models import Container
from MiravejaApi.Shared.DI.Exceptions import DependencyNameNotFoundInContainerException
...
class TestContainer:
    ...
    def test_FromAppConfigWithValidConfig_ShouldPopulateInstances(self):
        """Test that FromAppConfig creates container with AppConfig values in instances."""
        app_config = AppConfig(appName="Test App", appVersion="1.0.0", debug=True)

        container = Container.FromAppConfig(app_config)

        assert "appName" in container.instances
        assert "appVersion" in container.instances
        assert "debug" in container.instances
        assert container.instances["appName"] == "Test App"
        assert container.instances["appVersion"] == "1.0.0"
        assert container.instances["debug"] is True
