...
class MemberRoutes:
    ...
    def RegisterRoutes(
        ...
        async def ListAllMembersRoute(command: Optional[ListAllMembersCommand] = None):  # type: ignore
            return controller.ListAllMembers(command)

        @router.get(f"{BASE_ROUTE}/{{memberId}}", response_model=HandlerResponse)
        async def FindMemberByIdRoute(memberId: str):  # type: ignore
        ...
        async def RegisterMemberRoute(  # type: ignore
